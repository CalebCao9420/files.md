...
const _SearchModal = class _SearchModal {
  constructor() {
  ...
  init() {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (searchModal2.style.display !== "none" && !searchModal2.contains(event.target)) {
        this.close();
      }
    });
  }
  search() {
    ...
    performFileSearch(raw).then((results) => {
  ...
  close() {
    ...
    document.querySelectorAll(".message.actions-pinned").forEach((m) => m.classList.remove("actions-pinned"));
    this.selectedMsgElement = null;
  }
  showResults(results) {
    const list = document.getElementById("search-results");
    ...
    if (this.selectedMsgElement !== null) {
      ...
      const files2 = results.filter(({ path }) => path !== CONFIG_PATH && path !== CHAT_PATH);
      const rootFiles = files2.filter(({ path }) => toDirPath(path) === "/");
      const subFiles = files2.filter(({ path }) => toDirPath(path) !== "/");
  ...
  showRecentFiles() {
    ...
    results = results.sort((a, b) => b.lastModified - a.lastModified).slice(0, _SearchModal.RECENT_RESULTS);
...
_SearchModal.RECENT_RESULTS = 15;
let SearchModal = _SearchModal;
class MoveModal {
  constructor() {
    this.focusedIndex = 0;
    this.init();
  ...
  init() {
    ...
    document.addEventListener("click", (event) => {
      const moveModal2 = document.getElementById("move");
      if (moveModal2.style.display !== "none" && !moveModal2.contains(event.target)) {
        this.close();
      }
    });
  }
