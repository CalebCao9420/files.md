...
async function getTemporaryStorageDirHandle() {
  ...
  try {
    ...
    let seeded = true;
    try {
      await root.getFileHandle(lastFile);
    } catch {
      seeded = false;
    }
    if (seeded) {
...
function opfsIsFullyUsable() {
  ...
  return typeof proto.createWritable === "function" && typeof proto.remove === "function";
}
let memFSRoot = null;
...
function getHelpContent() {
  const stripMdFileLinks = (s) => s.replace(/^[ \t]*\[[^\]\n]+\]\([^)\n]*\.md\)[ \t]*\n?/gm, "").replace(/\n{3,}/g, "\n\n");
